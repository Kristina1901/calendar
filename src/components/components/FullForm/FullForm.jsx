...
export default function FullForm({fieldTitle, fieldMessage, fieldDay, fieldTime, findKey, onClose }) {
  ...
  const [time, setTime] = useState(fieldTime);
    ...
    return (
      <div className={s.container}>
        <p>Edit idea item</p>
        <span className={s.date}>{`Created at ${moment(new Date(date)).format("DD.MM.YYYY")} ${time}`}</span>
        ...
        <label className={s.tooltip}>Title*
         ...
         <label className={s.tooltip}>Date
          ...
          <button type="submit" className={s.button} onClick={(e) => handleSubmit(e)}>
           Save
           </button>
           <button type='button' className={s.delete} onClick={() => handleDelet()}>
           Delete
           </button>
           </form>
      </div>
